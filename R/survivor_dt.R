...
survivor_dt <- function(data,risk_data,event,time,breakpoint){
  ...
  for (i in 1:length(unique(data[,get(risk_data)]))){
    subset <- data[data[,get(risk_data)]==unique(data[,get(risk_data)])[order(unique(data[,get(risk_data)]))][i],]
  ...
  return(data.table(level=level_name,est=est,upper=upper,lower=lower))
}
